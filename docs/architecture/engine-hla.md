...
- Should client synchronization use an initial full Presentation Model or state snapshot on connection, followed by delta-only updates thereafter? This may improve responsiveness and reduce repeated transfer, but the exact contract belongs in Detailed Design after HLA-CONTRACT and HLA-QUERY interfaces are refined.
